...
**Note on Zammad search shape:** Tasks 4-5 were authored against an assumed two-step `/tickets/search` (id list) → `/tickets/{id}` (created_at) response. **This was corrected during the final gate (commit `496f690`):** live verification against the production Zammad 7.0.1 showed `/tickets/search` returns a **top-level JSON array of full ticket objects** (each with `id` + `created_at`), so the implementation parses that array directly in a single call — the second `/tickets/{id}` fetch was removed. The query field names (`customer_id`, `group.name`, `state.name`) and Elasticsearch availability were verified live. Treat the single-call array parse as the source of truth, not the two-step code blocks above.
